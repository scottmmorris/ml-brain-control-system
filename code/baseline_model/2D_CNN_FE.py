...
conv_channel_num	= 32
# pooling parameter
pooling_height_1st 	= 2
pooling_width_1st 	= 2

pooling_height_2nd 	= 2
pooling_width_2nd	= 2

pooling_height_3rd 	= 2
pooling_width_3rd 	= 2

pooling_height_4th 	= 2
pooling_width_4th 	= 2
...
# dataset directory
dataset_dir = "d:/ToRNeuralNetork/Cascade-Parallel-master/"

# load dataset and label
with open(dataset_dir+str(begin_subject)+"_"+str(end_subject)+"_shuffle_dataset_com_14_ICA.pkl", "rb") as fp:
  	datasets = pickle.load(fp)
with open(dataset_dir+str(begin_subject)+"_"+str(end_subject)+"_shuffle_labels_com_14_ICA.pkl", "rb") as fp:
  	labels = pickle.load(fp)

# reshape dataset
datasets = datasets.reshape(len(datasets), feature_num, 1)
...
conv_4_shape = str(kernel_height_4th)+"*"+str(kernel_width_4th)+"*"+str(kernel_stride)+"*"+str(conv_channel_num*8)
pool_4_shape = str(pooling_height_4th)+"*"+str(pooling_width_4th)+"*"+str(pooling_stride)+"*"+str(conv_channel_num*8)
...
output_dir 	= "2d_conv_3l_"+str(begin_subject)+"_"+str(end_subject)+"_fc_"+str(fc_size)+"_"+regularization_method+"_"+str(train_test_split*100)
output_file = "2d_conv_3l_"+str(begin_subject)+"_"+str(end_subject)+"_fc_"+str(fc_size)+"_"+regularization_method+"_"+str(train_test_split*100)
...
print(conv_1.shape[0])
print(conv_1.shape[1])
print(conv_1.shape[2])
print(conv_1.shape[3])

pool_1 = cnn_2d.apply_max_pooling(conv_1, pooling_height_1st, pooling_width_1st, pooling_stride)

print(pool_1.shape[0])
print(pool_1.shape[1])
print(pool_1.shape[2])
print(pool_1.shape[3])

# second CNN layer
conv_2 = cnn_2d.apply_conv2d(pool_1, kernel_height_2nd, kernel_width_2nd, conv_channel_num, conv_channel_num*2, kernel_stride)
pool_2 = cnn_2d.apply_max_pooling(conv_2, pooling_height_2nd, pooling_width_2nd, pooling_stride)

# third CNN layer
conv_3 = cnn_2d.apply_conv2d(pool_2, kernel_height_3rd, kernel_width_3rd, conv_channel_num*2, conv_channel_num*4, kernel_stride)
pool_3 = cnn_2d.apply_max_pooling(conv_3, pooling_height_3rd, pooling_width_3rd, pooling_stride)

# fourth CNN layer
conv_4 = cnn_2d.apply_conv2d(pool_3, kernel_height_4th, kernel_width_4th, conv_channel_num*4, conv_channel_num*8, kernel_stride)
pool_4 = cnn_2d.apply_max_pooling(conv_4, pooling_height_3rd, pooling_width_3rd, pooling_stride)

# flattern the last layer of cnn
shape = pool_4.get_shape().as_list()
pool_4_flat = tf.reshape(pool_4, [-1, shape[1]*shape[2]*shape[3]])

# fully connected layer
fc = cnn_2d.apply_fully_connect(pool_4_flat, shape[1]*shape[2]*shape[3], fc_size)
...
###########################################################################
	...
	writer = pd.ExcelWriter(result_dir+output_dir+"/"+output_file+"009.xlsx") # pylint: disable=abstract-class-instantiated

...
dataset_dir = "d:/ToRNeuralNetork/Cascade-Parallel-master/parallel_cnn_rnn/raw_data/"

with open(dataset_dir+"1_2_shuffle_cnn_dataset.pkl", "rb") as fp:
  	cnn_datasets = pickle.load(fp)
with open(dataset_dir+"1_2_shuffle_rnn_dataset.pkl", "rb") as fp:
  	rnn_datasets = pickle.load(fp)
with open(dataset_dir+"1_2_shuffle_labels.pkl", "rb") as fp:
...
rnn_in = tf.placeholder(tf.float32, shape=[None, n_time_step], name='rnn_in')
...
with tf.Session(config=config) as session:
	...
	writer = pd.ExcelWriter("./result/cnn_rnn_parallel/tune_rnn_layer/"+output_dir+"/"+output_file+".xlsx") # pylint: disable=abstract-class-instantiated
